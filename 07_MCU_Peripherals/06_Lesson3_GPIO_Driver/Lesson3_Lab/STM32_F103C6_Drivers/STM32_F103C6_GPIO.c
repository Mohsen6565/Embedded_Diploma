...
* Note			- None
*/
void MCAL_GPIO_WritePin(GPIO_Typedef_t* GPIOx, uint8_t pin_number, uint8_t value){
	if(value == GPIO_PIN_SET){
		SET_BIT(GPIOx->ODR, pin_number);
	}else{
		CLEAR_BIT(GPIOx->ODR, pin_number);
	}
}
...
* Note			- None
*/
uint8_t MCAL_GPIO_LockPin(GPIO_Typedef_t* GPIOx, uint8_t pin_number){
	uint8_t temp = 0 ;
	uint8_t return_value = 0 ;
// [1] Setting LCKK bit to 1
	SET_BIT(GPIOx->LCKR, 16);
// [2] Performing Locking Sequence
//	Write 1
	SET_BIT(GPIOx->LCKR, pin_number);
//	Write 0
	CLEAR_BIT(GPIOx->LCKR, pin_number);
//	Write 1
	SET_BIT(GPIOx->LCKR, pin_number);
//	Read 0
	temp = READ_BIT(GPIOx->LCKR, pin_number);
//	Read 1 (this read is optional but confirms that the lock is active)
	temp = READ_BIT(GPIOx->LCKR, pin_number);
	(temp == 1) ? (return_value = GPIO_PIN_LOCK_SUCCESS) : (return_value = GPIO_PIN_LOCK_FAILED) ;
